...
the following to a text file and feeding it to the receiver:
Example file config.txt:
<IPv4 mcast address> <port number>
<local IPv4 address>

Example execution of the program:
cat config.txt | ./receiver

The program proceeds by attempting to receive traffic from the multicast
group over that interface, and prints a line to standard output for each
message received, and from whom it was received.
...
void receive_data(const int sock, size_t mtu) {
  const size_t       buflen = (mtu > 0) ? mtu : DEFAULT_MTU;
  char               buf[buflen];
  int                res;
  socklen_t          t;
  ...
  do{
    res = recvfrom(sock, buf, buflen, 0, (struct sockaddr*)&remote, &t);
    if (res == -1) {
      perror("Failed in call to recvfrom");
    } else {
      printf("Received %i bytes from %x %x\n", res, remote.sin_addr.s_addr, remote.sin_port);
      memset(buf, 0, res);
    }
  }while(1);
}
...
int main() {
  ...
  /* Populate the structure with IPv4 address and port so that we can bind the socket. */
  struct sockaddr_in mcast, local;
  if (!populate_mcast_sockaddr_in(&mcast)) {
    return 0;
  }
  ...
  local.sin_port   = mcast.sin_port;

  if (!bind_socket(sock, &local)) {
    return 0;
  }

  if (!join_group(sock, &mcast_join)) {return 0;}

  receive_data(sock, mtu);
}
